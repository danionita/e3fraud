...
public class E3Model {
    ...
    private float lastKnownAverage;    
    private Map<Resource, Float> lastKnownAverages;
        ...
        while (startStimuli.hasNext()) {
            ...
            while (startStimulusFormulas.hasNext()) {
                ...
                if (formula.getString().split("=", 2)[0].equals("OCCURRENCES")) {
                    ...
                    //System.out.println("\t...found Start Stimulus \"" + startStimulus.getProperty(E3value.e3_has_name).getLiteral().toString() + "\", with OCCURENCES=" + occurences);
                }
            }
     ...
     */
    private void traverse(Resource nextElement, float occurences) {
    ...
    public float getLastKnownAverage(){
    ...
    public boolean changeNeedOccurrence(Resource need, float occurrence) {
     ...
     */
    public float getTotalForActorPerOccurence(Resource actor, Resource selectedValueInterface, boolean ideal) {
        float result = 0;
        ...
        if (actor.hasProperty(E3value.e3_has_formula)) {
            ...
            while (actorFormulas.hasNext()) {
                ...
                float value = Float.valueOf(formula.getString().split("=", 2)[1]);
                result -= value;
            }
        }
            ...
            while (actorValueInterfaces.hasNext()) {
                ...
                if (selectedValueInterfaces.contains(valueInterface.getResource())) {
                    ...
                    while (valueOfferings.hasNext()) {
                        ...
                        float valueOfferingValuation = 0;
     ...
     * @return the profit for that actor in the selected case
     */
    public float getTotalForActor(Resource actor, boolean ideal) {
        float result = 0;
        actor = model.getResource(actor.getURI());
    ...
    public XYSeries getTotalForActor(Resource actor, Resource need, int startValue, int endValue, boolean ideal) {
        ...
        try {
            ...
            float step = (endValue - (float) startValue) / 50;
...
return description;
    ...
    public float getCardinality(Resource res) {
        //make sure the resources are from this model
     ...
     * @param value
     */
    public void addTransfer(Resource valueInterface1, Resource valueInterface2, float value) {
        //make sure the resources are from this model
    ...
    void collude(Resource actor1, Resource actor2) {
        ...
        if (actor1.hasProperty(E3value.e3_has_formula)) {
            ...
            while (actor1Formulas.hasNext()) {
                ...
                float actor1FormulaValue = Float.parseFloat(actor1Formula.getString().split("=", 2)[1]);
                ...
                while (actor2Formulas.hasNext()) {
                    ...
                    float actor2FormulaValue = Float.parseFloat(actor2Formula.getString().split("=", 2)[1]);
                    //if it does
 ...
 * @return 
 */
    public float getAverageForActor(Resource actor, Resource need, int startValue, int endValue, boolean ideal) {
        ...
        for (Object dataItemObject : series.getItems()){
            ...
            sum +=dataItem.getY().floatValue();            
        }
        float mean = sum / series.getItemCount();        
     ...
     * @return 
     */
    Map<Resource, Float> getAveragesForActors( Resource need, int startValue, int endValue, boolean ideal) {
        
        need = model.getResource(need.getURI());
        Map<Resource, Float> averagesMap = new HashMap<>();
        ...
        float mean = sum / seriesMap.get(actor).getItemCount();        
    ...
    public Map<Resource, Float> getLastKnownAverages() {
